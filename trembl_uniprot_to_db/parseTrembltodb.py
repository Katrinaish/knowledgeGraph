...
sql = """INSERT INTO trembl_vertices(primary_accessID, uniprot_ID, geneName, geneSynonymList, 
        ...
        ncbiGeneID, intActID, stringID, ucscID, emblID, emblProtSeqID, protFullAltNameList, 
            ...
            if key == "ID":
                ...
                if len(values) > 1:
                    if (values[0] == 'GeneID' or values[0] == 'IntAct' or values[0] == 'KEGG' 
                        ...
                        key = key+values[0]
                        if len(values) > 2:
                            current[key+'ProtSeqID'].append(values[2].strip()) # protein sequence id
                        val = values[1]
